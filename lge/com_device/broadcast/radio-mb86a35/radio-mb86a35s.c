  ...
  int rtncode = 0;
   ...
   if (chipid0_rf == 0x03) {    //ES2.0 start
	...
	case PARAM_MODE_ISDBT_1UHF:
		swUHF = MB86A35_SELECT_UHF;
		#if 1//LGE
		isdbt_hw_set_rf_mode(0); //LGE for ear antenna
		#endif
		break;
	...
	case PARAM_MODE_ISDBTSB_3VHF:
		swUHF = MB86A35_SELECT_VHF;
		#if 1//LGE
		isdbt_hw_set_rf_mode(1); //LGE for ear antenna
		#endif
		break;
...
void mb86a35s_irq_work(struct work_struct *work)
{
	...
	struct ISDBT_MMBI_CTRL *pIsdbtWorkData;
	int free_size = 0;
	...
	//max 4K
		if(read_size >= 188*20)
		{
			...
			count = read_size/(188*20);
			remain_size = read_size%(188*20);
			...
			for(i=0;i<count;i++)
			{
				...
				/* Stream Read	*/
				ret = mb86a35s_spi_stream_recv(MB86A35S_REG_ADDR_STREAMREAD, _bufp, 188*20);
				if (ret != 0) {
				...
				if(free_size >= (188*20))
				{
					ret = mbt_dataring_write(buf_id, _bufp +3, 188*20); //header skip
					if(ret < 0)
					{
		...
		else
		{
			...
			if(free_size >= read_size)
			{
				...
				if(ret < 0)
				{
					printk("data ring buffer write fail5 %d \n", ret);
				}
			}
			else
			{
