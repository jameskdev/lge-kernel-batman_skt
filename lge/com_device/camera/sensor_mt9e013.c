...
#define CAMERA_LOG_DISABLE
 ...
 #endif
 #define CDBG(fmt, args...) do { } while (0)
 #define myCDBG(fmt, args...) printk(KERN_EMERG fmt, ##args)
#endif
...
static int32_t mt9e013_read_5100k_data(void)
{
	...
	myCDBG("[CAMERA][EEPROM] %s : Start : 0x%04x 0x%04x\n", __func__, raddr, waddr);
	n=0x01;
	for(j=0;j<5;j++) {
...
static int32_t mt9e013_write_exp_gain(uint16_t gain, uint32_t line)
{
	...
	int32_t rc = 0;
#if 1
    myCDBG("mt9e013_write_exp_gain entering.... \n");
...
#endif
	myCDBG("[CAMERA][1] mt9e013_write_exp_gain : frame_length_lines = NONE, mt9e013_ctrl->fps_divider = %d, mt9e013_ctrl->pict_fps_divider = %d\n", 
												/*frame_length_lines, */mt9e013_ctrl->fps_divider, mt9e013_ctrl->pict_fps_divider);	
	myCDBG("[CAMERA][2] mt9e013_write_exp_gain : line=%d gain=%d\n",line, gain); 
...
static int32_t mt9e013_sensor_setting(int update_type, int rt)
{
	...
	myCDBG("[CAMERA]mt9e013_sensor_setting :::: update_type = %d, rt = %d\n",update_type, rt );
	mt9e013_stop_stream();
	msleep(5);	// sungsik.kim@lge.com  2011-11-03  To Reduce Shutter-lag
	if (update_type == REG_INIT) {
		myCDBG("[CAMERA]mt9e013_sensor_setting :::: REG_INIT\n");
		...
		if (rt == QTR_SIZE) {
			printk(KERN_EMERG "[CAMERA] QTR_SIZE\n");
			...
			printk(KERN_EMERG "[CAMERA] FHD_SIZE\n");
			// jungki.kim@lge.com  2012-01-05  Not in use due to advanced C-AF
			//mt9e013_set_default_focus(0);	// jungki.kim@lge.com  2011-11-03  release Auto Focus when switching mode except capture.
			...
			printk(KERN_EMERG "[CAMERA] FULL_SIZE\n");
			// sungsik.kim@lge.com  2011-11-03  To Reduce Shutter-lag {
			//mt9e013_i2c_write_w_table(mt9e013_regs.reg_pll,	mt9e013_regs.reg_pll_size);
			//msleep(1);
			// sungsik.kim@lge.com  2011-11-03  To Reduce Shutter-lag }
...
static int32_t mt9e013_video_config(int mode)
{
	...
	myCDBG("[CAMERA]mt9e013_video_config :::: mode = %d, prev_res = %d\n",mode, mt9e013_ctrl->prev_res );
	...
	int res)
{
	...
	myCDBG("[CAMERA][KERNEL]mt9e013_set_sensor_mode ::: mode = %d, res = %d\n", mode, res);
...
{
	myCDBG("[CAMERA]mt9e013_power_down\n");
...
static int mt9e013_read_eeprom_data(struct sensor_cfg_data *cfg)
{
	...
	myCDBG("[CAMERA][QCTK_EEPROM] Product version = 0x%x bresult:%d\n", eepromdata,bresult);	
...
static int mt9e013_probe_init_sensor(const struct msm_camera_sensor_info *data)
{
	...
	if (!rc) {
		myCDBG("[CAMERA] %s : sensor_reset = %d\n", __func__, rc);
...
init_probe_done:
	myCDBG("[CAMERA] mt9e013_probe_init_sensor finishes \n");
	return rc;
}
...
int mt9e013_sensor_open_init(const struct msm_camera_sensor_info *data)
{
	...
	myCDBG("[CAMERA]mt9e013_sensor_open_init\n");
...
// LGE_DOM_UPDATE_S sungsik.kim 2011/07/23 {
	...
	myCDBG("[CAMERA]mt9e013_sensor_open_init(Power ON!)\n");
	if (rc < 0) {
...
static int mt9e013_init_client(struct i2c_client *client)
{
	...
	myCDBG("[CAMERA]mt9e013_init_client\n");
...
{
	int rc = 0;
	myCDBG("[CAMERA]mt9e013_i2c_probe\n");
...
static int mt9e013_sensor_release(void)
{
	...
	// LGE_DOM_UPDATE_S sungsik.kim 2011/07/23 {
	mt9e013_ctrl->sensordata->pdata->camera_power_off();
	myCDBG("[CAMERA]mt9e013_sensor_release(Power OFF!)\n");
	// LGE_DOM_UPDATE_E sungsik.kim 2011/07/23 }
#endif
