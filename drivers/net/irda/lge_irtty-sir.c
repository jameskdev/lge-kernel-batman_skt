...
int msm_irda_ldo_power_on(int );
int msm_irda_level_shift_ldo_power_on(int );
...
static int irtty_open(struct tty_struct *tty)
{
	...
	*/
	...
	msm_irda_ldo_power_on(1);
	if(lge_bd_rev >= LGE_REV_F ){msm_irda_level_shift_ldo_power_on(1);}
...
static void irtty_close(struct tty_struct *tty)
{
	...
	*/
	...
	msm_irda_ldo_power_on(0);
	if(lge_bd_rev >= LGE_REV_F ){ msm_irda_level_shift_ldo_power_on(0); }
...
int msm_irda_ldo_power_on(int on/*, bool log_en*/)
{
	int rc = -EINVAL;
	static struct regulator *vreg_l14;


	printk(KERN_INFO "[changyong.yi] %s: power %s\n", __func__, on ? "On" : "Off");

	vreg_l14 = regulator_get(NULL, "8058_l14");		/* +3V0_TOUCH */
	if (IS_ERR(vreg_l14)) {
		pr_err("%s: regulator get of 8058_l14 failed (%ld)\n", __func__, PTR_ERR(vreg_l14));
		rc = PTR_ERR(vreg_l14);
		return rc;
	}
	rc = regulator_set_voltage(vreg_l14, 2950000, 2950000);
...
int msm_irda_level_shift_ldo_power_on(int on/*, bool log_en*/)
{
	int rc = -EINVAL;
	static struct regulator *vreg_l20;


	printk(KERN_INFO "[changyong.yi] %s: power %s\n", __func__, on ? "On" : "Off");

	vreg_l20 = regulator_get(NULL, "8058_l20");		/* +1.8V0_TOUCH */
	if (IS_ERR(vreg_l20)) {
		pr_err("%s: regulator get of 8058_l20 failed (%ld)\n", __func__, PTR_ERR(vreg_l20));
		rc = PTR_ERR(vreg_l20);
		return rc;
	}
	rc = regulator_set_voltage(vreg_l20, 1800000, 1800000);

	if(on)
	{
		rc = regulator_enable(vreg_l20);
	}
	else
	{
		rc = regulator_disable(vreg_l20);
	}
