...
#if 0
static void max17047_quick_start(void)
{
	//Need to be update.
}
#endif
...
int max17040_get_mvolts(void)
{
	...
	//if (max17040_nobattery)
	//	return 3950;

	ret = max17047_multi_read_data(max17047_i2c_client, 0x02, &upper_reg, 1);
	if (ret < 0)
		return 800;

	ret = max17047_multi_read_data(max17047_i2c_client, 0x03, &lower_reg, 1);
	if (ret  < 0)
		return 800;

	vbatt_mv = ((((upper_reg<<8) | lower_reg)>>4) * 125) / 100;
	printk("%s: DCM vbatt_mv is %d mV\n", __func__, vbatt_mv);
...
int max17040_get_capacity_percent(void)
{
	...
	ret = max17047_multi_read_data(max17047_i2c_client, 0x04, &battery_soc, 1);
	if (ret < 0)
			return pre_soc;
...
int max17047_get_capacity_percent(void)
{
	...
	battery_soc = ((upper_reg *256)+lower_reg)*100/(95*256);

	if (battery_soc >= 100) battery_soc = 100;

	pre_soc = battery_soc;

	printk("%s: b325 batt_soc = %d (upper_reg = %d lower_reg = %d )\n", __func__, battery_soc, upper_reg, lower_reg);
...
static int max17047_new_custom_model_write(void)
{
	...
	max17047_write_reg(max17047_i2c_client, MAX17047_I_CHG_TERM, 0x0280);
...
bool max17047_quick_start(void)
{
	...
	max17047_i2c_write_and_verify(MAX17047_FULL_CAP, 0x1194);
...
EXPORT_SYMBOL(max17047_set_battery_atcmd);
			...
			const struct i2c_device_id *id)
{
	...
	//MAX17047 Full Memory DUMP
	#if 0
	for(i = 0; i< 0x100; i++)
		{
			read_reg = max17047_read_reg(max17047_i2c_client, i);
			printk("[MAX17047] MEM_DUMP Addr = 0x%X  Value = 0x%X \n", i, read_reg);
		}
	#endif
