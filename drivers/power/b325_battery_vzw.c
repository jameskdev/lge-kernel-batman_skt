...
extern void max17047_set_battery_atcmd(int flag, int value);
...
//	printk("[spiderman] charger_write : 0x%x[0x%x]\n", reg, value);
	if (ret < 0)
...
//	printk("[spiderman] charger_read : 0x%x[0x%x]\n", reg, ret);
	if (ret < 0)
	{
...
bool b325_charger_temp_control(int bat_temp)
{
	...
	case CHAR_TEMP_SCENARIO_45TO55_UNDER4000: //550mA
		...
		{
			reg_val = charger_read_reg(battery_dev->client, BQ24160_REG_CTRL);
			c_func("[325_BAT]%s CHAR_TEMP_SCENARIO_SUCCESS BQ24160_REG_CTRL  = %X \n", __func__, reg_val);
			reg_val = reg_val & 0x7D;//Enable CE.
			charger_write_reg(battery_dev->client, BQ24160_REG_CTRL, reg_val);
			charging_state = TRUE;

			printk(KERN_INFO "[325_BAT] Battery Temp = %d Normal temp continue Charging \n", bat_temp);
		}
		return 1;
...
static int b325_battery_data(void)
{
	...
	d_bat("[325_BAT] %s() battery updated - %d%%, %dmV, %d'C  Present = %d	Valid = %d\n",__func__,
...
int is_camera(void)
{
	...
	printk("[spiderman] is_camera() enabled : %d", iEnabled);
	return iEnabled;
}
#endif
...
static int b325_charger_data(void)
{
	...
	if ( usb_chg_type_for_FG == 2){ //TA
		...
		reg_read |= BQ24160_SUPPLY_SEL_MASK;
		charger_write_reg(battery_dev->client, BQ24160_REG_STAT_CTRL, reg_read);	
#else
...
#if defined(CONFIG_MACH_LGE_325_BOARD_VZW)
		charger_write_reg(battery_dev->client, BQ24160_REG_VINDPM_STAT, 0xDB);
#else
...
#ifdef	TRICKLE_CHARGING
			reg_read = charger_read_reg(battery_dev->client, BQ24160_REG_SAFETMR_NTCMON);
			if(battery_dev->batteryVoltage <= 3000)	{
				printk("trickle enter %d, reg=0x%X\n", battery_dev->batteryVoltage, reg_read);
				fast_charge_current = 0x00;
				reg_read = reg_read | 0x01;
			}
			else	{
				printk("trickle disable %d, reg=0x%X\n", battery_dev->batteryVoltage, reg_read);
				reg_read = reg_read & 0xFE;
			}
			charger_write_reg(battery_dev->client, BQ24160_REG_SAFETMR_NTCMON, reg_read);
#endif
		charger_write_reg(battery_dev->client, BQ24160_REG_BATTTERM_FCHGCRNT, fast_charge_current);
		...
		{
#ifndef CONFIG_BATMAN_VZW_POWER_BATT_THRM_SCENARIO
			charging_state = TRUE;
#endif
			...
			#if defined(CONFIG_MACH_LGE_325_BOARD_VZW)
				charger_write_reg(battery_dev->client, BQ24160_REG_VINDPM_STAT, 0xD2);
			#else
		...
		if (charging_state == TRUE) //Charger setting change to Default mode.
			{
				if ((lge_bd_rev >= LGE_REV_B) && (battery_check == TRUE))//Change the Input source to USB from Rev.D. by default
				{
					...
					if ((usb_cable_info != 6)&&(battery_dev->batteryVoltage > 3300)) //Blocking Port swtiching with 56K cable & Extremely Low battery
					{
						reg_read = (reg_read | BQ24160_SUPPLY_SEL_MASK);
						charger_write_reg(battery_dev->client, BQ24160_REG_STAT_CTRL, reg_read);
					}
	...
	d_bat("[325_BAT] %s() Battery = 0, USB = 1, AC = 2	  Charger Type = %d \n",__func__, chargerType);
...
static ssize_t at_fuel_guage_reset_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int r = 0;
#ifdef CONFIG_BATTERY_325_DCM
	max17047_set_battery_atcmd(0, 100);  // Reset the fuel guage IC
#else
